...
process them into L2A products using Sen2Cor and then save them to file. Then, a blob storage container is created
with a name matching the tileID. The individual images are uploaded into the blob container and then removed from local
storage. If a container already exists with name matching tileID, then the blobs are appended to the existing container.

Since there is intermediate local storage (all dates in range for each tile) some preplanning is required to avoid
running out of memory. For the ES20_v3 Linux VM with 160GB local storage, recommend setting a date range of up to 1 month
at a time. The number of tiles is irrelevant because the local store is flushed of files before downloading images for
each tile.
...
def process_L1C_to_L2A(L1C_path, L1Cfiles, unzip_files=True):
    ...
    for L1C in L1Cfiles:
        ...
        if unzip_files:
            ...
            files = glob.glob(str(L1Cpath + '*.zip'))  # find files containing "L1C"
            for f in files:
                os.remove(f)

        # process L1C to L2A
        sen2cor = str(
            '/home/tothepoles/PycharmProjects/Sen2Cor-02.05.05-Linux64/bin/L2A_Process ' + L1C_path + '{}'.format(
                L1C) + ' --resolution=20')
        os.system(sen2cor)
...
def send_to_blob(blob_account_name, blob_account_key, tile, L1Cpath, check_blobs=False):
    ...
    if check_blobs:
        ...
        upload_status = 1  # if check_blobs deselected, assume upload was successful (not recommended)
...
alltiles = ['22XDF', '21XWD', '21XWC', '22WEV', '22WEU', '22WEA', '22WEB', '22WED', '21XWB', '21XXA', '21WEC', '21XVD',
            '22WES', '22VER', '22WET']
...
# loop through tiles
tiles = ['22WED', '21XWB', '21XXA', '21WEC', '21XVD', '22WES', '22VER', '22WET']
...
for i in np.arange(0, len(tiles), 1):
    ...
    try:
        ...
        print("TILE ID {} FINISHED".format(tile))
        print(' MOVING TO NEXT TILE '.center(80, 'X'), '\n', 'X'.center(80, 'X'))

    except:
        print("No images found for this tile on the specified dates")

print(' FINISHED ALL TILES '.center(80, 'X'), '\n', 'X'.center(80, 'X'))